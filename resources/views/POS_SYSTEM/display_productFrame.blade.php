    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6" id="productsGrid">
        @forelse($products as $product)
            <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition product-card cursor-pointer"
                ...
                data-quantity="{{ $product->stock_quantity ?? 0 }}"
                data-price="{{ $product->price ?? 0 }}"
                onclick="addToCart({{ $product->id }}, '{{ addslashes($product->product_name) }}', {{ $product->price }}, '{{ addslashes($product->serial_number) }}', '{{ addslashes($product->brand?->brand_name ?? 'N/A') }}', '{{ addslashes($product->category?->category_name ?? 'N/A') }}', '{{ $product->product_condition }}')">
                
                <div class="p-4 border-b {{ $product->product_condition === 'Brand New' ? 'bg-green-50' : 'bg-orange-50' }}">
                    <div class="flex justify-between items-start">
                        <span class="text-xs font-semibold px-2 py-1 rounded-full {{ $product->product_condition === 'Brand New' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800' }}">
                ...
                <div class="p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-800">{{ $product->brand?->brand_name ?? 'N/A' }}</span>
                        </div>
                        
                        <div class="flex justify-between">
                            <span class="text-gray-600 font-medium">Category:</span>
                            <span class="text-gray-800">{{ $product->category?->category_name ?? 'N/A' }}</span>
                        </div>
                        
                        <div class="flex justify-between">
                            <span class="text-gray-600 font-medium">Serial No:</span>
                            <span class="text-gray-800 font-mono text-xs">{{ $product->serial_number }}</span>
                        </div>
                        
                        <div class="flex justify-between">
                            <span class="text-gray-600 font-medium">Warranty:</span>
                            <span class="text-gray-800">{{ $product->warranty_period ?? 'N/A' }}</span>
                        </div>
                        
                        <div class="flex justify-between">
                            <span class="text-gray-600 font-medium">Stock:</span>
                            <span class="text-gray-800">{{ $product->stock_quantity }} available</span>
                        </div>
                    </div>
                </div>
            </div>
